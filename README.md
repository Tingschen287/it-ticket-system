...
> 🤖 本项目由 **Claude Code Agent** 持续性自主开发完成，采用 Anthropic 推荐的长期运行 Agent 架构。

## 持续性 Agent 开发架构

本项目采用 **Anthropic 推荐的长期运行 Agent 架构**，由 Claude Code 自主完成全部开发工作。

### 核心架构文件

```
it-ticket-system/
├── init.sh                  # 启动脚本（启动开发服务器）
├── feature_list.json        # 功能需求列表（JSON格式，追踪功能状态）
├── claude-progress.txt      # 进度日志（记录每个session的工作）
└── .git/                    # Git仓库（追踪所有代码变更）
```

### Agent 工作流程

每个开发 Session 中，Agent 会自动执行以下流程：

```
┌─────────────────────────────────────────────────────────────┐
│                    Agent 开发循环                            │
├─────────────────────────────────────────────────────────────┤
│  1. 读取 claude-progress.txt 了解上次进度                    │
│  2. 读取 git log 查看最近的提交                              │
│  3. 读取 feature_list.json 找到下一个未完成的功能            │
│  4. 运行 init.sh 启动开发服务器，验证现有功能                 │
│  5. 实现一个新功能                                          │
│  6. 编译测试验证                                            │
│  7. git commit 提交代码                                     │
│  8. 更新 claude-progress.txt 进度日志                        │
│  9. 更新 feature_list.json 标记功能为 passes: true           │
│  10. 循环执行直到所有功能完成                                │
└─────────────────────────────────────────────────────────────┘
```
...
1. **渐进式开发** - 每次只实现一个功能，确保质量
2. **自动化验证** - 编译测试通过后才提交
3. **完整记录** - 进度日志和 Git 提交记录所有变更
4. **可恢复性** - 任何时刻中断都能从上次进度继续
